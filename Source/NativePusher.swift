    ...
    */
    private func modifySubscription(interest: String, change: SubscriptionChange, successCallback: @escaping (Any?) -> Void) {
        guard pusherAppKey != nil && clientId != nil else {
            self.delegate?.debugLog?(message: "pusherAppKey \(String(describing: String(describing: pusherAppKey))) or clientId \(String(describing: clientId)) not set - waiting for both to be set")
